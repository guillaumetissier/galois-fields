...
### Fixed
- **Polynomial field validation**: `assertSameField()` now correctly compares fields by order instead of object identity
  - Two distinct `GaloisField(256)` instances are now recognized as equivalent
  - Fixes issue where polynomials from different field instances (but same order) would incorrectly throw `InvalidArgumentException`

## [1.3.0] - 2025-02-03

### Added
- **BitString-Polynomial conversion support**: Polynomials can be converted to BitString and vice versa
  - `PolynomialConverter` — Convert to/from BitString / binary strings
